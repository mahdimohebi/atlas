            ...
            <div>
                <nav>
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">ویرایش پرداخت</li>
                    </ol>
                </nav>
            </div>
        </div>
                ...
                <div class="card bg-light p-3">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            @endphp
                            <p>نهایی: <strong>{{ number_format($totalPrice) }} AFN</strong></p>
                            <p>پرداخت شده قبل از این پرداخت: <strong>{{ number_format($totalPaidExcludingCurrent) }} AFN</strong></p>
                            <p>مقدار این پرداخت: <strong>{{ number_format($payment->amount) }} AFN</strong></p>
                            <p>باقی‌مانده بعد از ویرایش: 
                                <strong id="remaining_after">{{ number_format($totalPrice - $totalPaidExcludingCurrent - $payment->amount) }}</strong> AFN
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card custom-card">
                    <div class="card-body">
                        <form action="{{ route('customer_payment.update', $payment->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row gy-4">
                                ...
                                <div class="col-md-6 col-sm-12">
                                    ...
                                    <input type="text" id="amount" name="amount" class="form-control" value="{{ old('amount', $payment->amount) }}" required>
                                    @error('amount')
                                ...
                                <div class="col-12 text-center">
                                    <input type="submit" class="btn btn-primary" value="به‌روزرسانی پرداخت">
                                </div>
...
window.onload = function() {
    ...
    const paidExcludingCurrent = {{ $totalPaidExcludingCurrent }};
    ...
    amountInput.addEventListener('input', function() {
        ...
        let payment_new = parseFloat(raw.replace(/,/g, '')) || 0;
        let remaining = total - paidExcludingCurrent - payment_new;
